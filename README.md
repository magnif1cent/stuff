...
- **Tags**: members choose from the existing list or type a new one directly in the add/edit form (verified email required, rate-limited). New tags go live immediately — tags aren't treated as data-hygiene-critical, so there's no approval queue — with a case-insensitive check against the existing list ("Weapon Duel"/"weapon duel" resolve to the same tag) to keep near-duplicates from piling up. Admins manage the full list (rename, delete, see usage counts) at `/admin/fight-scene-tags` as the fallback for anything that slips through.
