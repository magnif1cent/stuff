...
- **Sifu Lineage**: admin-curated martial arts training lineages, connecting actors already in the catalog and non-actor figures alike — its own tree-view section on the actor page (direct sifu and students) links out to a full, multi-generation tree (see [Sifu Lineage](#sifu-lineage) below)
...
When an actor has a martial arts lineage recorded, its own **Lineage** section — a small tree, not a stat card — appears further down the page, between the Details/Sparring Partner/bio row and Known For — see [Sifu Lineage](#sifu-lineage) below.

## Sifu Lineage

Martial artists often trace their training back through a chain of sifus, sometimes generations deep. Admins can record that lineage — and not every sifu is an actor in the catalog: a historical martial artist never credited in a film, or a character like Ip Man (played by different actors across different films), can be recorded too.

- **Data model** — a lineage node is a `LineageFigure`, not a `Person` directly: it has a name and an optional unique `personId` linking it to an actor. An actor's figure is created automatically the moment they're first linked (searching and picking an actor "just works"); a non-actor figure is added by name and deduped by exact name if entered again. `LineageRelation` links a `sifuId` to a `studentId` (both figures) with an optional short note (e.g. "Wing Chun", "adopted disciple"). A student can have more than one sifu recorded (it's a DAG, not a strict tree), but exactly one is `isPrimary` at a time — the first one recorded becomes primary automatically, and it's the one that positions the student in the rendered tree; any additional sifu renders as a secondary "co-sifu" line instead of an equal branch. This was a deliberate choice over full graph layout — see DECISIONS.md.
- **Admin management** (`/admin/lineage`, `ADMIN`-only, see [Admin Area & Roles](#admin-area--roles) below) has three ways to add a link: a **bulk chain-import** box (paste one succession chain per line, `Old Master Yuen > White Crane Elder > Iron Fist Chen`, sifu first — a chain of N names becomes N−1 links, with a review step that flags names matching more than one actor or matching none before anything is saved; this path stays actor-only) — the fast path for entering a whole lineage at once; a plain **single-link form** for one-off corrections; and directly on the **lineage tree** itself, via a "+ Sifu"/"+ Student" button under whichever figure is centered, which searches (actors and existing non-actor figures together, plus an "add as a non-actor figure" option) and saves in one step. Every add path rejects a link that would create a cycle.
- **Public view** — every actor page carries its own **Lineage** section (`LineageTreeBody`, the same tree renderer the full page below uses) when they have at least one link recorded: their direct sifu, any co-sifus, and direct students, drawn as a small tree rather than a list, with a **"View full lineage →"** link to the full, multi-generation tree. An actor-linked figure's tree lives at `/actors/[personId]/lineage`; a non-actor figure (no actor page to live under) gets its own page at `/lineage/[figureId]`. Clicking any figure in the tree re-centers it on them. A non-actor figure with no photo of its own shows who's played them on screen where known — looked up live from the catalog's existing cast-credit data (`characterName`), not stored, since more than one actor can plausibly have played the same figure. Every public view carries a short disclaimer that lineage links are compiled to the best of the site's knowledge and subject to correction.
...
- **Lineage** (`/admin/lineage`, `ADMIN`-only) &mdash; link actors (and non-actor figures, added by name) into martial arts training lineages, via bulk chain-paste, a single-link form, or directly on the lineage tree (see [Sifu Lineage](#sifu-lineage) above).
