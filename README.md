...
Biography, birthday, and place of birth (when TMDB has them) are shown beside the Career Highlights card, live-fetched via their `person.tmdbId` on each page view rather than stored in our own database — if TMDB is unreachable or has nothing on record, that section is simply omitted. Sharing the row with Career Highlights leaves the biography a narrower column than it used to have, so it now clamps to 10 lines with its own **"Show more"/"Show less"** toggle once it's long enough to need one — the same clamp-with-toggle mechanics already used for member reviews and the homepage's Recent Reviews by Editors (see [Reviews](#reviews)), just with more room before the toggle kicks in, since a biography reads more like an article than a review.
