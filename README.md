...
- Registration intentionally still reveals whether an email is already registered (`"An account with that email already exists"`) rather than a generic response — CAPTCHA above already neutralizes the bulk-harvesting risk a generic response would guard against, so hiding it would just cost real signup UX for no real gain. See `DECISIONS.md` for the full reasoning, including why registration no longer signs the member in and why sign-in itself now requires a verified email.
