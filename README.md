...
Just under the actor's name, a **Details** card (the same bordered dt/dd box as the movie page's Studio/Country card) summarizes their career from data already gathered for the rest of the page — no extra queries: filmography size, total fight scenes tagged, average community rating across their filmography, average rating across their tagged fight scenes, years active (earliest–latest release year among their movies), and, when they've shared at least 2 fight scenes with the same other actor, a **Sparring Partner** row linking to whoever that is. Any row without a value (e.g. no ratings yet, or no co-star clearing the 2-scene minimum) is simply omitted, same as the movie page's Details card.
