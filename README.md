...
- **Meme Generator** (`/admin/memes`, `ADMIN`-only) &mdash; caption an image with a classic top/bottom meme layout and download it as a PNG. The base image is either a fight scene's linked YouTube video thumbnail (searched by scene or movie title, proxied server-side through `/api/admin/memes/thumbnail` so the canvas export isn't cross-origin-tainted) or a screenshot dropped/browsed in directly, which always overrides the scene thumbnail when present. Everything is composited client-side on a `<canvas>` &mdash; nothing is uploaded, stored, or shared from here.
