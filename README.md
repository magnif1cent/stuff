...
- **Descriptions, ranking, and notes**: "Edit list" on a list's own permalink opens two pills, **Details** (name and an optional description, up to 280 characters — same cap as a member's profile bio) and **Rank my list**, where the owner turns ranking on or off. Off (the default) keeps the classic view — movies then fight scenes, two flat grids, sorted by when each was added. On replaces both grids with a single numbered "reel" that merges movies and fight scenes into one ranked sequence — a specific fight scene can outrank a whole film, or vice versa — each row tagged FILM or FIGHT so the type is still visible. The owner reorders with per-row up/down controls (not drag-and-drop) and can attach a short note (up to 240 characters) to any item, shown as a pull-quote under it. Ranking is per-list, not per-item — turning it off doesn't discard any saved order or notes, it just stops the page from showing them.
