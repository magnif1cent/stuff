...
- **Mobile filter sheet on `/search/fights`**: below the `sm:` breakpoint, the sidebar filter form (title, tags, actor, ratings, genre, country, year range, sort) isn't shown inline — a **Filters** button sits in the quick-filter bubble row instead (badged with how many filter groups are currently active) and opens the same form as a bottom sheet, with Apply pinned to the bottom so it's reachable without scrolling through every field. Closes via its own × button, the backdrop, or Escape. At `sm:`+ this is a no-op: the identical form renders as the ordinary static sidebar, same as before the sheet existed — there's exactly one copy of the form either way, not two kept in sync.
