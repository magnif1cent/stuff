...
Just under the actor's name, a **Career Highlights** card sits to the left of their bio, summarizing their career from data already gathered for the rest of the page — no extra queries: filmography size, total fight scenes tagged, average community rating across their filmography, average rating across their tagged fight scenes, years active (earliest–latest release year among their movies), and, when they've shared at least 2 fight scenes with the same other actor, a **Sparring Partner** stat linking to whoever that is. Styled with the same gold left-accent stripe and 🏆 kicker pill as the Signature Vote spotlight banner below (see Signature Role/Signature Fight Scene further down this section), rather than the movie page's neutral Details card — a deliberate choice to make this read as a tribute to the actor rather than plain reference data. Any stat without a value (e.g. no ratings yet, or no co-star clearing the 2-scene minimum) is simply omitted, and an actor with none of these stats gets no card at all.
...
Biography, birthday, and place of birth (when TMDB has them) are shown beside the Career Highlights card, live-fetched via their `person.tmdbId` on each page view rather than stored in our own database — if TMDB is unreachable or has nothing on record, that section is simply omitted. Sharing the row with Career Highlights leaves the biography a narrower column than it used to have, so it now clamps to 4 lines with its own **"Show more"/"Show less"** toggle once it's long enough to need one — the same clamp-with-toggle pattern already used for member reviews and the homepage's Recent Reviews by Editors (see [Reviews](#reviews)).
