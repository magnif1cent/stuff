...
- **Movie-page teaser**: only the newest 2 scenes render on the movie page itself, fetched that way server-side rather than fetched-in-full-then-hidden — a movie with many scenes doesn't ship every scene's cast/tags/ratings to the browser just to show two of them. A "View all N fights" link below the teaser leads to the full collection.
