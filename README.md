...
- **`/search/fight-scenes`** — filters: category tag (multi-select, matches any selected), actor (autocomplete, scoped to people actually tagged in a fight scene — not just anyone in a movie's cast), minimum member rating, minimum editor rating, and the fight's movie's release-year range. Sort by newest, highest member rated, highest editor rated, or most favorited. Unlike `/search` (movies), which needs a query or filter before showing anything, this page shows every fight scene by default (paginated, newest first) — it's titled "Browse fight scenes," so landing on it with no input actually browses instead of prompting for one.
- **Quick-filter bubbles** above the results on `/search/fight-scenes` — one-click shortcuts into a filtered/sorted view (Top Rated, Most Favorited, and every category tag) for the values that don't need free-text input, faster than opening the sidebar form. The open-ended actor filter stays sidebar-only, since a bubble row can't represent free text the way it can a fixed set of tags/sort orders.
