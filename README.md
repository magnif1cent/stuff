...
Further down the page, between Lineage (below) and Known For, a **Collaborators** section lists this actor's most frequent co-stars — ranked by shared movies (`APPROVED` only), then by shared fight scenes, with no minimum threshold and no blended score, since a ranked list makes no claim about who "the" partner is the way a single named card would. Drawn as a row of avatars branching off the actor's own portrait via a connecting line, reusing the same visual language as the Lineage tree below; the top-ranked match's connector and avatar ring pick out in red. Every collaborator links to a pairwise page at `/actors/[personId]/with/[otherPersonId]` listing every shared movie (with each side's character name, where recorded) and shared fight scene. Replaces an earlier single "Sparring Partner" card, which only looked at fight-scene tagging and required 2+ shared scenes before naming anyone — most actors, including ones with a substantial filmography, showed nothing at all under that design (see DECISIONS.md).
...
Biography, birthday, and place of birth (when TMDB has them) fill the rest of that same row, live-fetched via their `person.tmdbId` on each page view rather than stored in our own database — if TMDB is unreachable or has nothing on record, that section is simply omitted. Sharing the row with the Details card leaves the biography a narrower column than it used to have, so it now clamps to 10 lines with its own **"Show more"/"Show less"** toggle once it's long enough to need one — the same clamp-with-toggle mechanics already used for member reviews and the homepage's Recent Reviews by Editors (see [Reviews](#reviews)), just with more room before the toggle kicks in, since a biography reads more like an article than a review.
...
When an actor has a martial arts lineage recorded, its own **Lineage** section — a small tree, not a stat card — appears further down the page, between the Details/bio row and Collaborators (above) — see [Lineage](#lineage) below.
