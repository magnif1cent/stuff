...
- **`/tops/fights`** — the 100 highest community-rated fight scenes, same idea but as "Fight Ticket"-styled cards (the same cream ticket look used elsewhere for fight scenes) with a stamped rank badge in the corner instead of a numeral over a poster. Paginated 24 per page through the same shared `Pagination` component used everywhere else, so the rank badge stays the scene's true overall rank (not reset to 1 on each page) and jumping to e.g. page 3 doesn't require paging through 1-2 first.
