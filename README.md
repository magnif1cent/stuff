...
- **List actions**: everything you can do to a list sits in one actions panel beside it on desktop (below 1024px wide, a ⋯ button under the title opens the same actions as a bottom sheet). The owner sees whether the list is private or public, its like and item counts, **Edit list**, **Make this list private/public**, **Copy link** (public lists only), and **Delete list…** (asks to confirm, then returns to the owner's profile). A visitor sees **Like this list**, the counts, **Clone this list** (non-empty lists only), and **Copy link**; on a phone their Like button stays visible next to the ⋯ button.
