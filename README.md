...
- **Quick-filter bubbles** above the results on `/search/fights` — one-click shortcuts into a filtered/sorted view (Top Rated, Most Favorited, Verified only, My Favorites when signed in, a Jackie Chan actor shortcut, and a movie-decade shortcut for each of 70s/80s/90s/2000s Era, using the same `yearFrom`/`yearTo` range the sidebar form takes) for the values that don't need free-text input, faster than opening the sidebar form. Category tags aren't duplicated here — the sidebar's own Tags checkboxes already cover that filter. The actor filter is otherwise sidebar-only (open-ended text, not a fixed set of values a bubble row can represent), and likewise the movie genre/country dropdowns. A **Clear** link sits directly in that bubble row (whenever a bubble-controlled value is active) resetting the whole search, so turning an active bubble back off doesn't require scrolling down to the sidebar form's own Clear link next to Apply.
