...
- **Quick-filter bubbles** above the results on `/search/fights` — one-click shortcuts into a filtered/sorted view (Top Rated, Most Favorited, Verified only, My Favorites when signed in, and a movie-decade shortcut for each of 70s/80s/90s/2000s Era, using the same `yearFrom`/`yearTo` range the sidebar form takes) for the values that don't need free-text input, faster than opening the sidebar form. Category tags aren't duplicated here — the sidebar's own Tags checkboxes already cover that filter. The open-ended actor filter and the movie genre/country dropdowns stay sidebar-only, since a bubble row can't represent free text or a longer option list the way it can a handful of fixed sort/era/boolean shortcuts.
