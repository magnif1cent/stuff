...
Registering with email/password creates the account and sends a verification link, but does not sign you in. Signing in with a password also requires a verified email — `/login` rejects a correct password on an unverified account with "Verify your email before signing in," and shows a "Resend verification email" control right there (posts to the unauthenticated `/api/resend-verification-public`, so it works with no session). Once signed in, the same verified-email requirement still gates rating movies, managing lists, and posting in discussions — that gate matters for an already-signed-in account whose email gets un-verified later (an admin changing their own sign-in email resets it), not just at registration; a banner with its own "Resend email" button (the authenticated `/api/resend-verification`) appears until you verify. Google sign-ins are auto-verified, since Google already confirmed that address, and are signed in immediately as before.
...
- **`/search/fights`** — filters: category tag, martial arts style, and named move (each multi-select, matches any selected), actor (autocomplete, scoped to people actually tagged in a fight scene — not just anyone in a movie's cast), the fight's movie's genre, country, and release-year range, minimum member rating, minimum editor rating, admin-verified only, and (signed-in only) favorited by me. Sort by newest, highest member rated, highest editor rated, or most favorited. Unlike `/search` (movies), which needs a query or filter before showing anything, this page shows every fight scene by default (paginated, newest first) — it's titled "Browse fight scenes," so landing on it with no input actually browses instead of prompting for one.
- **Quick-filter bubbles** above the results on `/search/fights` — one-click shortcuts into a filtered/sorted view (Top Rated, Most Favorited, Verified only, My Favorites when signed in, a Jackie Chan actor shortcut, and a movie-decade shortcut for each of 70s/80s/90s/2000s Era, using the same `yearFrom`/`yearTo` range the sidebar form takes) for the values that don't need free-text input, faster than opening the sidebar form. Each bubble is a standalone view, not a filter that layers onto whatever's already selected — clicking one replaces the whole query, and clicking an already-active bubble again deselects it, toggling back to the plain, unfiltered page. Category tags aren't duplicated here — the sidebar's own Tags checkboxes already cover that filter. The actor filter is otherwise sidebar-only (open-ended text, not a fixed set of values a bubble row can represent), and likewise the movie genre/country dropdowns. A **Clear** link also sits directly in that bubble row (whenever a bubble-controlled value or a sidebar-only filter is active) resetting the whole search, so it's not just the active bubble itself that has an obvious way back to unfiltered.
- **Pagination** on both search pages (and every other paginated list in the app — a movie's Fights/Reviews, `/lists`, an actor's Tributes, `/news`, a Timeline era) goes through one shared `Pagination` component (`src/components/pagination.tsx`): Previous/Next plus clickable page numbers, collapsed to first/last/current-±1 with an ellipsis once there are more than 7 pages, so jumping straight to e.g. page 4 doesn't require paging through 1-2-3 first.
- **Mobile filter sheet on both search pages**: below the `sm:` breakpoint, the sidebar filter form isn't shown inline on either page — a **Filters** button (badged with how many filter groups are currently active) opens the same form as a bottom sheet instead, with Apply pinned to the bottom so it's reachable without scrolling through every field. On `/search/fights` the button sits in the quick-filter bubble row; on `/search` (no bubble row there) it sits next to the results heading. Closes via its own × button, the backdrop, or Escape. At `sm:`+ it's a no-op on both pages: the identical form renders as the ordinary static sidebar, same as before the sheet existed — there's exactly one copy of the form either way (`components/filter-sheet.tsx`), not two kept in sync.
...
`/admin/import` has five ways to find and import movies — search-and-browse (title, keyword, actor, or studio) for discovering films, or CSV upload when you already know exactly what you want:

- **By title** — search TMDB by movie title and import one result at a time, with a poster thumbnail shown alongside each result so you can confirm you've got the right film (or cut, for titles TMDB has more than one of) before importing. Good for a specific film you already know by name.
- **By keyword** — search for one or more TMDB keywords (e.g. "kung fu", "martial arts"), optionally narrow by production country (a curated dropdown of common origins like Hong Kong, China, Taiwan — TMDB's `with_origin_country` accepts any ISO 3166-1 code, the dropdown just picks common ones) and/or a release-year range (From/Year, next to the country dropdown — either end can be left blank), then browse matching movies (20 per page, "Load more" to page further) with poster, title, release year, production country, and top-billed cast shown for each, so you can judge relevance before importing. Selecting multiple keywords matches movies tagged with *any* of them (TMDB's `with_keywords` OR logic), not all of them; the country and year filters both AND against that (TMDB's `primary_release_date.gte`/`.lte`, a whole-year span from Jan 1 to Dec 31). Results are pre-checked by default — uncheck the ones that don't belong rather than checking the ones you want — and movies already in your catalog are shown but excluded from selection. "Import selected" imports the checked movies (a few at a time, not all at once) and reports how many succeeded.
  - TMDB's `/discover/movie` endpoint (used for keyword and actor search) caps at page 500 (10,000 results) regardless of how many total matches it reports; a search with more matches than that needs narrowing (e.g. an additional keyword, a country filter, or a year range) to reach everything.
...
- **By actor** — search TMDB for a person by name, pick the right match from the (photo-and-department-labeled) results, then browse their filmography the same way as keyword search: same country and year-range filters, same paged result cards (poster, title, year, country, top-billed cast), same pre-checked/already-in-catalog/batch-import behavior. Uses TMDB's `with_cast` discover filter for one person at a time (not an OR/AND set like keywords). Shares its result-browsing, filters, and batch-import UI and backend (`/api/admin/tmdb/discover`, now also accepting a `personId`) with By keyword — the two only differ in how the movie list itself is filtered.
- **By studio** — search TMDB for a production company by name, pick the right match from the (logo-and-origin-country-labeled) results, then browse everything TMDB has attributed to that studio the same way as keyword/actor search: same filters, same result cards, same pre-checked/already-in-catalog/batch-import behavior. Uses TMDB's `with_companies` discover filter for one company at a time. Shares its result-browsing, filters, and batch-import UI and backend (`/api/admin/tmdb/discover`, now also accepting a `companyId`) with By keyword and By actor.
...
- **Custom lists are public by default, and can be made private; Favorites/Watchlist are always private.** Every custom list has its own shareable permalink at `/lists/[id]` (also reachable via its owner's profile) that anyone can view, signed in or not. The owner can choose "Make this list private" from the list's actions to hide it from everyone else: the permalink then 404s for anyone but the owner, the like and clone APIs treat it as not found, and it drops out of `/lists`, the leaderboard (Most-Liked Lists and Top Curators' movie counts), the Community Activity feed, other members' view of the owner's profile, and likers' Liked tabs. Existing likes are kept but hidden while it's private, and come back if it's made public again. The owner's own Lists tab marks private lists with a "Private" badge. Favorites and Watchlist (for both movies and fight scenes) stay exactly as private as they've always been: only the signed-in owner ever sees their own, on their own profile or anywhere else.
...
- **Descriptions, ranking, and notes**: every list — ranked or not — renders as the same row layout: a thumbnail (a fight scene's carries a small play-icon overlay, since it's a video clip rather than a page like a movie's poster), a FILM/FIGHT badge next to the title (with a matching color accent on the row's left edge), rating, and, for the owner, a note and a remove control on every row. Ranking (off by default) is the only thing that changes what's shown: on, each row also gets its position number and per-row reorder controls (move up, move down, move to top, move to bottom), merging movies and fight scenes into one ranked sequence — a specific fight scene can outrank a whole film, or vice versa. Off, rows are just sorted by when each was added. The owner turns ranking on or off with a **Ranked list** switch on the list's edit page (`/lists/[id]/edit`, owner-only, reached from "Edit list"), alongside the name and an optional description (up to 280 characters — same cap as a member's profile bio). The owner can also attach a short note (up to 240 characters) to any item, ranked or not, shown as a pull-quote under it — a row without one yet shows a **+ Add a note** link in its place rather than only a small pencil icon, so the option is visible before there's anything to edit; once a note exists, the pencil sits right next to it. Ranking is per-list, not per-item — turning it off doesn't discard any saved order or notes, it just stops the page from numbering by them. When a fight scene's own movie is also separately in the same list, its row notes the relationship ("also #3 in this list" when ranked, "also in this list" when not) instead of leaving it implicit — the two are related, even though they're two different entries. On narrow screens, a row's owner controls (reorder buttons, remove) drop to their own line below the title instead of squeezing into the same row as the thumbnail and text, which had no room left for them at phone widths.
- **Profile Lists tab shows one card per list, not the lists' items**: both your own Lists tab and another member's show each list as a single card — a small cover collage of its first few posters/clips (the same cover as `/lists` browse cards), the list name (which links to the list), Private/Ranked badges, the description clamped to two lines ("No description" if there isn't one), and a line of counts: items, movies, fights, likes, and when it was last updated. Cards sit two across on wider screens and stack on a phone. On your own profile each card also has Rename and Delete. The page loads only a few cover tiles per list, so it stays light no matter how many lists a member has or how big they get; the list's own page (`/lists/[id]`) shows everything.
- **Liking lists and the leaderboard**: any signed-in, verified member other than the list's own owner can like a public custom list (one like per member per list; self-likes are blocked). `/leaderboard` ranks the Most-Liked Lists and, separately, Top Curators (members with the most total movies across their own lists), Most Beloved Actors (actors ranked by `PersonFavorite` count — see [Actor Pages](#actor-pages)), and Top Franchises (TMDB collections with at least 2 approved movies in the catalog, ranked by the average of every individual community rating across all their movies — a straight weighted average, so a collection's one-star outlier with only a couple of ratings doesn't count as much as its other entries' hundreds; a collection needs at least one rating anywhere in it to appear at all). All four rankings recompute on every page load rather than being cached/scheduled. It cross-links with [`/tops`](#tops) (highest community-rated movies and fight scenes) — a related but separate "rankings" surface, not folded in here since it charts individual movies/fight scenes rather than lists/curators/actors/franchises.
- **List actions**: everything you can do to a list sits in one actions panel beside it on desktop. The owner sees whether the list is private or public, its like and item counts, **Edit list**, **Make this list private/public**, **Copy link** (public lists only), and **Delete list…** (asks to confirm, then returns to the owner's profile). A visitor sees **Like this list**, the counts, **Clone this list** (non-empty lists only), and **Copy link**. Below 1024px wide the panel becomes one row of labeled buttons under the title — **Edit**, **Make private/public** and **Share** for the owner, **Like · count**, **Clone** and **Share** for a visitor. Share opens the phone's own share sheet where the browser supports it, otherwise it copies the link. Delete isn't in that row; it's at the bottom of the edit page (`/lists/[id]/edit`), which has it at every screen size.
- **Adding items from the list itself**: on your own list's page, a search box above the items ("Add a movie or fight to this list…") finds movies (by title, cast member or director, same matching as the navbar search) and fight scenes (by the fight's title or its movie's title, top-rated first with the rating shown), up to 6 of each, and adds one with a tap. Results show inline under the box; an ✕ clears them. Anything already in the list shows "In list ✓" instead of an Add button. On a ranked list, an item you just added shows where it landed ("Added as #9" — new items go to the bottom) with a one-tap **Move to top**; on an unranked list it just says "Added ✓", since new items show first there anyway. The box stays open so you can add several from one search. Only approved movies and non-deleted fight scenes appear. It's backed by `GET /api/lists/[id]/search`, owner-only (404 for anyone else), and adds through the same entry endpoints as the add-to-list control on movie and fight pages, so the 200-item cap and ranking behave the same. Items can still be added from each movie's or fight's own page too.
...
- **Browsing lists**: `/lists` — reachable via the "Lists" nav link — lists every public custom list with at least one item, sorted by newest-updated or most-liked, 24 per page in a dense grid (2 columns on mobile up to 6 on desktop). Each small card has a Spotify-playlist-style cover collage of up to 4 poster/thumbnail tiles from the list's own contents (a single square for a 1-item list, a 2x2 grid for 2 or 4, and a dedicated asymmetric layout — one tall tile plus two stacked — for exactly 3, so it doesn't leave a visibly empty fourth cell), its name, owner, item count, and like count; a fight-scene tile in the collage gets the same small play-icon overlay as its row does elsewhere. A **Ranked** badge sits directly on a ranked list's cover so it's identifiable from the card alone, not just from which section heading it happens to be grouped under. A search box filters by list name or owner username. Results on each page are split into "Ranked" and "Unranked" sections (using the same `isRanked` flag every list already has) whenever that page actually has both kinds — a page of all-unranked lists (the common case) shows no section headers at all. It cross-links to `/leaderboard` and vice versa, so both surfaces are reachable from one another. A chevron next to the navbar's "Lists" link also reveals `/leaderboard` directly in a small click-to-open submenu, without needing to go through the Lists page first.
...
- **Each search result shows a poster and top-billed cast** to help pick the right match before submitting — the poster comes free with TMDB's title search, but cast doesn't, so this costs one extra `getTmdbMovieDetails` call per result shown (same tradeoff, and the same `extractTopBilledCast` helper, as the admin "By keyword"/"By actor" import tabs below).
...
- **Collection page** (`/movies/[id]/fights`): every non-deleted scene for the movie, with the same grid and "+ Add fight scene" form as the movie page teaser used to have before it was capped. Pagination is real (`?page=N`, 6 scenes per page — the browser never loads more than one page's worth of cast/tags/ratings), not the client-side "Show more" the old in-place list used. Quick-filter bubbles above the grid sort (Round Order, Newest First, Highest Rated, Most Favorited) and filter (Verified only, any tag actually used on this movie's own scenes) — same pattern as `/search/fights`, scoped to one movie.
- **Add a scene**: paste a YouTube URL (any watch/shorts/embed/`youtu.be`/live link, with an optional `t=`/`start=` timestamp used as the clip's initial start time), give it a title (the submitter can auto-fill from the video's public oEmbed title), tag which of the movie's own cast members are in it, and optionally attach up to 10 category tags, up to 5 martial arts styles, and up to 10 named moves. Requires a verified email.
...
- **Tags**: members choose from the existing list or type a new one directly in the add/edit form (verified email required, rate-limited). New tags go live immediately — tags aren't treated as data-hygiene-critical, so there's no approval queue — with a case-insensitive check against the existing list ("Weapon Duel"/"weapon duel" resolve to the same tag) to keep near-duplicates from piling up. A member-facing profanity check (`bad-words`) rejects an outright-profane name at creation time rather than silently censoring it; it doesn't catch merely silly/joke tags, which still rely on the admin fallback below. Admins manage the full list (rename, delete, see usage counts) at `/admin/fight-scene-tags` as the fallback for anything that slips through, and aren't subject to the profanity check themselves — they're trusted to curate the vocabulary directly.
- **Style & Move**: two more data points members can attach to a scene — the martial arts style(s) on display (e.g. "Drunken Boxing") and the named move(s) thrown (e.g. "Flying Kick") — each rendered as its own colored badge row on the scene card (style in the rating-stamp red, move in olive), separate from category tags. Unlike tags, these stay admin/reviewer-curated only: a member searches and picks from the existing list via an autocomplete field (not a checkbox grid, since the lists can grow long) but can't create a new value inline — a "don't see it?" hint below each field points at asking an admin instead. Admins manage both lists (add, rename, delete, see usage counts) at `/admin/fight-scene-styles` and `/admin/fight-scene-moves`. See `DECISIONS.md` for why this stays closed-vocabulary despite tags having gone the other way.
- **Style groups**: admins can optionally cluster styles into named groups (e.g. "Northern", "Southern", "Internal") from the same `/admin/fight-scene-styles` page — a style's group is purely organizational and doesn't change how it's used. Where it shows up: the admin style list itself clusters styles under a group heading (ungrouped styles fall under "Ungrouped"); the style autocomplete field above shows a style's group next to its name in the suggestion dropdown; and the style filter on `/search/fights` clusters its checkboxes under a group heading (ungrouped styles fall under "Other"). Grouping headers only appear once more than one group actually exists — with no groups configured, all three surfaces look exactly as before. Not offered for Move, only Style. On `/admin/fight-scene-styles` a style can also be reassigned by dragging its row onto a group — either in the Groups management list above, or onto another cluster in the Styles list below (including "Ungrouped") — as a quicker alternative to the per-style dropdown, which stays as the keyboard-accessible fallback (confirmed working via touch on mobile too, not just desktop mouse).
...
- **Desktop** — a single horizontal axis with one dot per movie, positioned within its era's band; hovering a dot shows its title, era, year, and community rating (pure CSS hover, no JS needed for the tooltip itself). Band width tracks real historical duration from Warring States through Republic of China — except the five most recent eras (`POSTWAR_ERA` onward), which get deliberately more room per year than earlier ones, and Legendary/Shang/Spring & Autumn, which aren't drawn to real calendar scale at all (a small fixed bucket each, since together they'd otherwise cost hundreds of extra axis pixels for eras next to no catalog movie is ever set in). Both scale changes are marked with a visible axis-break rather than silently changing scale — without the modern one, the five eras where most of the catalog actually lives would inherit the same few cramped pixels a single "Modern" band would get under strict proportionality. An era with zero movies gets a small "No movies yet" label instead of just blank space under its name — a band with even one movie already gets a real dot, so this only ever fires for the genuinely empty case.
- **Era quick-jump chips** sit above the axis, one per era plus a "Today →" shortcut to Contemporary; clicking one scrolls the (very wide — 3229px at full width) axis straight to that band instead of hand-scrolling past everything in between. The chip for whichever era is currently in view highlights automatically as you scroll.
- **A minimap below the axis** mirrors the same band layout at a glance, proportional to the real per-era pixel widths, with each band's bar height showing roughly how many movies it holds (square-root scaled, so one 80-movie era doesn't flatten every sparse dynasty into invisible slivers). A red box marks the currently visible slice of the axis above — click or drag anywhere on the minimap to pan to a different part of the timeline.
- **A "Min. rating" filter dims non-matching movies in place** (lower opacity) rather than hiding them or navigating to a filtered list, so a dot's spatial/chronological position never moves under a filter change. Scoped to rating only, since that's the one per-movie facet the timeline's own query already fetches — a genre or verified-only filter would need that query to start joining more data.
- **A constant-interval (10-year) tick ruler runs under the Qing-through-Contemporary stretch of the axis** (`computeScaleTicks()` in `src/lib/timeline.ts`), replacing an earlier plain-text note as the way the modern axis-break gets disclosed. Because every tick is the same number of years apart, tick *density* shows the scale change on its own — packed tight before the break, spread out after it — rather than asking anyone to read a sentence. Scoped to Qing onward on purpose: every dynasty from Warring States through Qing already sits at one roughly-consistent px/year rate, so there's no OTHER scale change to show with a tick ruler. Legendary/Shang/Spring & Autumn aren't on a per-year scale at all, so that earlier break relies on its hatched marker alone, with no matching tick ruler to pair it with. Both hatched break markers are hoverable/keyboard-focusable and carry a one-line tooltip explaining what changed — opt-in, not a permanent caption, so it doesn't undo the "nothing to read unless you're curious" point of the tick ruler.
...
- Both charts are paginated 24 per page through the same shared `Pagination` component used everywhere else, so the rank badge/numeral stays each item's true overall rank (not reset to 1 on each page) and jumping to e.g. page 3 doesn't require paging through 1-2 first.
- Both charts require at least 2 community ratings to qualify (same threshold and shape as the homepage's own "Top Rated by the Community" rail — `getTopRatedMovies`/`getTopRatedFightScenes`), computed fresh on every page load like every other ranking in the app, no cache/schedule.
- These are two separate pages rather than one page with two sections, since a movie poster grid and a fight-ticket grid don't share a layout. See the leaderboard bullet under [Member Lists & Profiles](#member-lists--profiles) for the app's other "rankings" surface (Most-Liked Lists, Top Curators, Most Beloved Actors, Top Franchises) — `/tops` cross-links with it rather than folding into it.
...
When an actor has a martial arts lineage recorded, its own **Lineage** section — a small tree, not a stat card — appears further down the page, between the Details/Sparring Partner/bio row and Known For — see [Lineage](#lineage) below.

## Lineage

Martial artists often trace their training back through a chain of teachers, sometimes generations deep. Admins can record that lineage — and not every earlier figure in it is an actor in the catalog: a historical martial artist never credited in a film, or a character like Ip Man (played by different actors across different films), can be recorded too.

- **Data model** — a lineage node is a `LineageFigure`, not a `Person` directly: it has a name, an optional list of `aliases` (other names/romanizations it's credited under), and an optional unique `personId` linking it to an actor. An actor's figure is created automatically the moment they're first linked (searching and picking an actor "just works"); a non-actor figure is added by name and deduped by exact name if entered again. `LineageRelation` links a `sifuId` to a `studentId` (both figures, naming aside — these are the earlier and later figure in a link) with an optional short note (e.g. "Wing Chun", "adopted disciple"). A figure can have more than one earlier link recorded (it's a DAG, not a strict tree), but exactly one is `isPrimary` at a time — the first one recorded becomes primary automatically, and it's the one that positions the figure in the rendered tree; any additional earlier link renders as a secondary line instead of an equal branch. This was a deliberate choice over full graph layout — see DECISIONS.md. Display copy avoids "sifu"/"student" wording throughout, since a specific role term doesn't fit every lineage recorded here — see DECISIONS.md.
- **Groups** — a non-actor figure can also be marked `isGroup` when it's added (a stunt team, say, rather than a single trained person). A group figure renders with its own square-and-icon node wherever it appears in the tree — as one distinctly-shaped entry in its owner's own descendant row, same as any individual — rather than a lateral or otherwise special-cased position; its own members are simply its primary students, one generation further down. A group's own roster is capped more generously than an individual's before the overflow badge kicks in (`DEFAULT_GROUP_SIBLING_LIMIT` in `lib/lineage.ts`), since a team can run far larger than any one person's students. See DECISIONS.md for why groups deliberately aren't a lateral "beside the owner" position.
- **Admin management** (`/admin/lineage`, `ADMIN`-only, see [Admin Area & Roles](#admin-area--roles) below) has three ways to add a link: a **bulk chain-import** box (paste one succession chain per line, `Old Master Yuen > White Crane Elder > Iron Fist Chen`, earliest first — a chain of N names becomes N−1 links, with a review step that flags names matching more than one actor or matching none before anything is saved; this path stays actor-only) — the fast path for entering a whole lineage at once; a plain **single-link form** for one-off corrections; and directly on the **lineage tree** itself, via a "+ Earlier"/"+ Later" button under whichever figure is centered, which searches (actors and existing non-actor figures together, plus an "add as a non-actor figure" option, with a checkbox to mark that new figure as a group) and saves in one step. Every add path rejects a link that would create a cycle. A bare figure centered on the tree also gets **"Mark/Unmark as group"** (corrects the flag on one added without it) and **"Delete figure"** (removes it and, via cascade, every link it was part of) — both refuse an actor-linked figure, which is auto-managed and not meant to be edited this way. Any centered figure (actor-linked or not) also gets an **"Also known as"** editor — add or remove alternate names/romanizations it's credited under (e.g. a nickname alongside its formal name), up to 10 — see **Public view** below for what these feed into.
- **Public view** — every actor page carries its own **Lineage** section (`LineageTreeBody`, the same tree renderer the full page below uses) when they have at least one link recorded: their direct earlier and later links, and any secondary links, drawn as a small tree rather than a list, with a **"View full lineage →"** link to the full, multi-generation tree. An actor-linked figure's tree lives at `/actors/[personId]/lineage`; a non-actor figure (no actor page to live under) gets its own page at `/lineage/[figureId]`. Clicking any figure in the tree re-centers it on them. When a figure has more children/co-sifus than fit (a sibling limit — more generous for a group's own roster than an individual's), a dashed **"+N more"** badge appears and is itself a link that re-centers the same tree with that limit raised just enough to reveal what was cut off. A non-actor figure with portrayal data (who's played them on screen, where known) gets a small numbered marker next to its name instead of inline text — every marker's actual detail (actor names, each linking to that actor's page, with the film's release year) is collected into one **Portrayed by** list below the whole tree, so a figure's node stays a fixed size no matter how much cast data it has. Looked up live from the catalog's existing cast-credit data (`characterName`, matched with spelling/punctuation variants normalized away) against the figure's name *and* any aliases recorded for it together, not stored, since more than one actor can plausibly have played the same figure, and the same figure can be credited under more than one name across different films (a nickname, a different romanization) — see **Admin management** above for adding aliases. On the two full-tree pages (not the actor-page teaser), the tree sits in a fixed-height, pinch-zoom/pan-able viewport (`LineageTreeZoom`, via `react-zoom-pan-pinch`) with on-screen +/− /reset controls, rather than the plain horizontal scroll the teaser uses — a tree several generations deep can be taller and wider than any one screen at once, and no single fixed scale can stay both readable and show the whole shape; see DECISIONS.md. Every public view carries a short disclaimer that lineage data is hand-curated and a work in progress.
...
- **Fight Styles** (`/admin/fight-scene-styles`) and **Fight Moves** (`/admin/fight-scene-moves`) &mdash; manage the two closed vocabularies behind the Style/Move data points (see [Fights](#fights) above): add, rename, delete, see usage counts. Fight Styles also lets admins organize styles into optional groups (add, rename, delete a group; assign a style to one or none). Both pages open to `REVIEWER`, same as Fight Scene Tags.
- **Lineage** (`/admin/lineage`, `ADMIN`-only) &mdash; link actors (and non-actor figures, added by name) into martial arts training lineages, via bulk chain-paste, a single-link form, or directly on the lineage tree (see [Lineage](#lineage) above).
...
- **Meme Generator** (`/admin/memes`, `ADMIN`-only) &mdash; caption an image with a classic top/bottom meme layout, then download it as a PNG or copy it straight to the clipboard (`navigator.clipboard.write`, shown only when the browser actually supports writing images, not just text). The base image is either a fight scene's linked YouTube video thumbnail (searched by scene or movie title, proxied server-side through `/api/admin/memes/thumbnail` so the canvas export isn't cross-origin-tainted) or a screenshot dropped/browsed in directly, which always overrides the scene thumbnail when present. Everything is composited client-side on a `<canvas>` &mdash; nothing is uploaded, stored, or shared from here.
...
- **Rate limiting**: `src/lib/rate-limit.ts` throttles the endpoints most worth throttling — login (5 attempts / 5 min, keyed by email), registration, forgot-password, and the unauthenticated resend-verification (5 / 10 min, keyed by IP), the authenticated resend-verification (3 / hour, keyed by user), and the content-creation endpoints (discussion posts, fight scene submissions, movie submissions, list creation — 10-20 / 10 min, keyed by user). Login failures caused by rate limiting look identical to a wrong password (a generic `CredentialsSignin` error) so the limiter doesn't leak its own state to an attacker. Backed by [Upstash Redis](https://upstash.com) via `@upstash/ratelimit`, so limits are enforced correctly across serverless instances rather than reset per cold start. Without `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` configured, rate limiting is a no-op — every request is allowed, not blocked — so local dev and CI work unchanged. To enable it, create a free database at [console.upstash.com](https://console.upstash.com), copy its REST URL/token into `.env`, and restart the dev server.
- **CAPTCHA**: registration, forgot-password, and the unauthenticated resend-verification form (shown on `/login` after an unverified sign-in attempt, and on `/verify-email`'s expired-link state) all render a [Cloudflare Turnstile](https://developers.cloudflare.com/turnstile/) widget and verify the token server-side — the three unauthenticated flows most attractive to automated bulk abuse (mass account creation, mass password-reset/verification email spam). Not added to login itself — that's already covered by the per-email rate limit above, and a CAPTCHA on every mistyped password would just be friction for legitimate members. Without `NEXT_PUBLIC_TURNSTILE_SITE_KEY`/`TURNSTILE_SECRET_KEY` configured, no widget renders and no verification is required — local dev and CI work unchanged. To enable it, create a free widget at [the Cloudflare dashboard](https://dash.cloudflare.com/?to=/:account/turnstile) and copy its site key/secret key into `.env`.
- **Password requirements**: `src/lib/password.ts`'s `validateNewPassword()` — shared by registration, admin password change, and forgot-password reset so the rules can't drift between them — requires at least 8 characters (no composition rules like forced symbols/numbers, which mostly just push people toward predictable patterns) and caps it at 72 bytes (bcrypt silently truncates past that, so a longer password past this point isn't actually adding strength — capping it makes that real limit explicit instead of a silent surprise). Hashed with bcrypt at cost factor 12 (OWASP's current recommended minimum, bumped from 10).
- Registration no longer reveals whether an email is already registered — `/api/register` returns the same generic success response either way, silently doing nothing (no new account, no email) when the email already has one, matching `/api/forgot-password` and `/api/resend-verification-public`'s anti-enumeration shape. A taken username is still a real, immediate error, since usernames are public handles, not something to protect. See `DECISIONS.md` for the full reasoning, including why registration no longer signs the member in and why sign-in itself now requires a verified email.
