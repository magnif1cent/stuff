...
- **Not applied to `FightSceneMove`** — only Fight Styles were asked about, and Move has no equivalent grouping need raised yet. If that changes, `groupStylesByCategory()` in `src/lib/fight-scenes.ts` is written generically enough to reuse rather than being Style-specific.
...
- **No headers when there's nothing to distinguish.** `groupStylesByCategory()` returns one bucket when zero or one group exists; both surfaces render exactly as they did before this feature until an admin actually creates a second group — a deliberate no-op default rather than an "Other" heading appearing over what's really just one flat list.
