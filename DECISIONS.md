...
### Community Forum built as new standalone models, not an extension of DiscussionPost
**PR #TBD.** A general-purpose forum (`/forum`), separate from the existing per-movie discussion system, requested explicitly as its own space so it can grow independently without touching movie pages or discussion code at all.

- **`ForumCategory`/`ForumThread`/`ForumPost` are new models, not a relaxed `DiscussionPost`**: `DiscussionPost.movieId` is required and every existing call site relies on that — a forum post has no movie to anchor to, so making it optional there (or adding a generic polymorphic parent) would have meant reworking a constraint the whole discussion feature is built on, for a feature explicitly asked to stay separate. `ForumPost` mirrors `DiscussionPost`'s own shape instead (one level of flat replies, soft-delete, `[spoiler]` tags via the same `SpoilerText` component) so the two features read the same way without sharing a table.
- **A thread's starting post is an ordinary `ForumPost`, not inline content on `ForumThread`**: `ForumThread` holds no body text of its own — creating a thread creates the thread row and its first `ForumPost` (`parentId: null`) in one call. This means the starting post is editable/deletable through the exact same endpoint as every other post, with no special-cased "edit the thread" path.
- **Category CRUD opened to `REVIEWER`, not `ADMIN`-only**: considered matching News & Updates (`ADMIN`-only, since it's structural site content) but went with matching `FightSceneTag` instead — a forum board is the same shape of thing (an admin-curated list members choose from, not user-generated), and `FightSceneTag` already established that pattern as `REVIEWER`-accessible. `/admin/forum-categories` reuses that precedent directly rather than treating this as closer to News & Updates.
- **Category deletion blocked while non-empty, unlike `FightSceneTag`'s freely-deletable vocabulary**: a tag carries no history worth preserving (removing one just drops it from the scenes that used it); a forum category can hold real member discussion. `DELETE /api/admin/forum/categories/[id]` rejects with a specific error until every thread is deleted or moved out, rather than letting one click cascade-delete an entire board's content the way the schema's `onDelete: Cascade` would otherwise allow.
- **Forum moderation (pin/lock/delete-any-thread/delete-any-post) is `ADMIN`-or-`REVIEWER`, wider than discussion moderation's `ADMIN`-only delete**: a deliberate divergence from `DiscussionPost`'s existing precedent, per explicit direction to reuse both roles for this feature's moderation rather than narrowing to match discussion's current (`ADMIN`-only) behavior.
- **Locking stops new posts but leaves existing ones visible and editable**; pinned threads sort first within their category, then by most recent post activity (`ForumThread.updatedAt` is bumped on every new reply, not just on thread creation) — recently-active threads surface the same way a typical forum listing works, not just recently-started ones.
- **Rate-limited like discussion posts**: `forumThreadCreateLimiter` (10/10 min) and `forumPostLimiter` (20/10 min) added to `src/lib/rate-limit.ts`, same shape as `discussionPostLimiter`, keyed by user id.
